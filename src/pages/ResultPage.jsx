...
import { Stepper, Footer, ModeBadge } from './UploadPage'
...
export default function ResultPage() {
  ...
  const mode = sessionStorage.getItem('px_mode') || 'encrypt'

  useEffect(() => {
    const imageData = sessionStorage.getItem('px_image_data')
    const rotorIds  = JSON.parse(sessionStorage.getItem('px_rotors')    || '[1,2,3]')
    const positions = JSON.parse(sessionStorage.getItem('px_positions') || '[0,0,0]')
    const pairs     = JSON.parse(sessionStorage.getItem('px_pairs')     || '[]')
    if (!imageData) { navigate('/'); return }
    ...
    img.onload = () => {
      ...
      // Worker code inline sebagai string
      const workerCode = `
        ...
        function rotorFwd(x,A,B,pos){
          return mod256(affineEncrypt(mod256(x+pos),A,B)-pos)
        }
        function rotorBwd(x,A,B,pos){
          return mod256(affineDecrypt(mod256(x+pos),A,B)-pos)
        }

        self.onmessage=function(e){
          const{buf,width,height,rotors,positions,pairs}=e.data
          ...
          // Transfer buffer kembali ke main thread — tanpa copy
      ...
      worker.onmessage = (e) => {
        ...
        const resultImgData = new ImageData(resultData, img.width, img.height)
        ctx.putImageData(resultImgData, 0, 0)
        const rUrl    = canvas.toDataURL('image/png')
        const elapsed = ((performance.now() - start) / 1000).toFixed(2)
        setResultUrl(rUrl)
        setStats({
          Piksel:  (img.width * img.height).toLocaleString(),
          Dimensi: `${img.width} × ${img.height}`,
          Waktu:   elapsed + 's',
      ...
      // Transfer buffer langsung — TANPA Array.from(), TANPA copy
      // slice(0) membuat salinan buffer agar canvas original tidak terpengaruh
      const transferBuf = imgDataObj.data.buffer.slice(0)
      worker.postMessage({
        buf:       transferBuf,
        width:     img.width,
        height:    img.height,
        ...
        positions,
        pairs,
      }, [transferBuf])  // ← Transferable: zero-copy ke Worker
    }
  ...
  const handleDownload = () => {
    ...
    a.href = resultUrl
    a.download = `pixenigma_${mode}_result.png`
    a.click()
  }
        ...
        {status === 'processing' && (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'28px', flex:1, padding:'80px 0' }}>
            ...
            <div style={{ textAlign:'center' }}>
              <div style={{ fontFamily:'Syne, sans-serif', fontSize:'18px', fontWeight:800, letterSpacing:'0.15em', color:'#0A0A0A', marginBottom:'8px' }}>
                MEMPROSES GAMBAR
              </div>
              <div style={{ fontFamily:'Space Mono, monospace', fontSize:'11px', letterSpacing:'0.15em', color:'#555555', fontWeight:700 }}>
                Mengenkripsi piksel satu per satu...
              </div>
            </div>
          </div>
        )}
        ...
        {status === 'done' && (
          <>
            ...
            <div style={{ display:'flex', gap:'12px', flexWrap:'wrap', justifyContent:'center' }}>
              ...
              <button onMouseEnter={()=>setBtnHover('new')} onMouseLeave={()=>setBtnHover(null)} onClick={()=>navigate('/')} style={outlineBtn(btnHover==='new')}>
...
function DownloadSVG({color}){
  return(
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <path d="M7 1v8" stroke={color} strokeWidth="1.5" strokeLinecap="round"/>
      <path d="M4 7l3 3 3-3" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
...
function RefreshSVG({color}){
  return(
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <path d="M2.5 7a4.5 4.5 0 104.5-4.5c-1.5 0-2.8.7-3.7 1.8" stroke={color} strokeWidth="1.5" strokeLinecap="round"/>
      <path d="M3 2v3h3" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}
